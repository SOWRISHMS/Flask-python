...
app.config['SECRET_KEY'] = os.environ.get('SECRET_KEY', 'a_strong_and_secret_key_for_dev')
...
class User(db.Model):
    ...
    # Cascade: If a user is deleted, all their tasks are deleted
...
class Task(db.Model):
    ...
    due_date = db.Column(db.Date, nullable=True) # Stored as a Date object
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
...
def login_required(f):
    ...
    def decorated_function(*args, **kwargs):
        if 'user_id' not in session:
            # Using 'warning' category as it typically displays an orange alert
...
def parse_date(date_str):
    if date_str:
        try:
            # Parses string 'YYYY-MM-DD' into a date object
...
if __name__ == '__main__':
    # When running locally, set debug=True for automatic reloads
    app.run(debug=True)
